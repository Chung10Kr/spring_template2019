...
public class EgovWebUtil {
    ...
    */
    ...
    public static String getMenuGubnVal(HttpServletRequest request){
		String returnVal = "1";
		String cPath = request.getRequestURI(); // 요청 URI
		
		if(cPath.startsWith("/cms")){
			returnVal = "1";
		}else if(cPath.startsWith("/web")){
			returnVal = "2";
		}else if(cPath.startsWith("/eng")){
			returnVal = "3";
		}
		return returnVal;
	}
	...
	/**
	 * 7,8,9는 파일업로드 jfile 사용 그외는 jquery 파일업로드 사용
	 * @param request
	 * @return
	 */
	public static boolean getBrowserCheckFileUi(HttpServletRequest request) {
		String brInfo = getBrowserinfo(request);
		if(brInfo.equals("Internet Explorer 7") || brInfo.equals("Internet Explorer 8") || brInfo.equals("Internet Explorer 9")){
			return true;
		}
		return false;
	}
